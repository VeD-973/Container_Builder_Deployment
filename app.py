...
def place_nonH(x,y,z,colors,nH_list,container,ax,curr_weight,stored_plac,vol_occ,y_min,df,vol_wasted,storage_strip): # Used for placing the non homogenous boxes
    width_container = float(container.width)
    height_container = float(container.height)
    ...
    return  x,y,z,vol_occ,y_min,df,vol_wasted,storage_strip,nH_list  ## Remember to add vol_occ calculation in the nonHomo 
...
def perform_computation(df,container_toFit,strip_list,key,roll):
    ...
    x,y,z,vol_occ,y_min,df,vol_wasted,storage_strip,nH_list = place_nonH(x,y,z,colors,nH_list,container_toFit,ax,curr_weight,stored_plac,vol_occ,y_min,df,vol_wasted,storage_strip)
    if y_min <0:
        y_min = 0
...
def worker(df, container_toFit,strip_list,keys,roll,container_num):
    ...
    # Adjust the viewing angle for bottom view
        ...
        filename = os.path.join('static', f"bottom_view_iteration_final_{container_num}.png")
        plt.savefig(filename)
